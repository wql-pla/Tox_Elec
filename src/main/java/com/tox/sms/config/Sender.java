...
import com.tox.sms.config.HttpDeleteUtil;
import com.tox.sms.config.Message;
import com.tox.sms.config.RequestEncoder;
...
public class Sender implements ISender {
	...
	protected HttpEntity build(Map<String, Object> data) {
		...
		builder.addTextBody(APPID, config.getAppId(),contentType);
//		builder.setCharset(contentType);
		builder.addTextBody(TIMESTAMP, this.getTimestamp(),contentType);
		builder.addTextBody(SIGN_TYPE, config.getSignType(),contentType);
		// set the properties below for signature
		data.put(APPID, config.getAppId());
		data.put(TIMESTAMP, this.getTimestamp());
		data.put(SIGN_TYPE, config.getSignType());
		ContentType contentType1 = ContentType.create(HTTP.PLAIN_TEXT_TYPE, HTTP.UTF_8); 
